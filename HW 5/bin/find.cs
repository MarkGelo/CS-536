// Dafny program find.dfy compiled into C#
...
// Command Line Options: c:\Users\Gelo\Desktop\IIT\CS-536\HW 5\find.dfy /verifyAllModules /compile:1 /spillTargetCode:1 /out:bin\find
// find.dfy

method findFirstGreater(a: seq<int>, x: int) returns (i: int)
  ensures (0 <= i < |a| && forall j: int :: 0 <= j < i ==> a[j] <= x && a[i] > x) || i == |a|
  decreases a, x
{
  i := 0;
  while i < |a| && a[i] <= x
    invariant 0 <= i <= |a| && forall j: int :: 0 <= j < i ==> a[j] <= x
    decreases |a| - i, if i < |a| then x - a[i] else 0 - 1
  {
    i := i + 1;
  }
}
...
namespace _module {
  ...
  public partial class __default {
    public static BigInteger findFirstGreater(Dafny.ISequence<BigInteger> a, BigInteger x)
    {
      BigInteger i = BigInteger.Zero;
      i = BigInteger.Zero;
      while (((i) < (new BigInteger((a).Count))) && (((a).Select(i)) <= (x))) {
        i = (i) + (BigInteger.One);
      }
      return i;
    }
